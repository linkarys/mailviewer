...
<cfif (structKeyExists(form,"deleteALL"))>
...
<script language="javascript" type="text/javascript">
	...
	function getMyEmail(filename) {
		var href = "bottom.cfm?mail=" + filename,
			xmlhttp = new XMLHttpRequest(),
			eviewer = document.getElementById('eviewer');
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				eviewer.innerHTML = xmlhttp.responseText;
				eviewer.style.right = '55px';
				eviewer.style.opacity = 1;
			}
		}
	...
	document.addEventListener('keydown', function(e) {
		...
		if (e.which === 27) {
			eviewer.style.right = '-1000px';
			eviewer.style.opacity = 0;
		}
	}, false)
...
<style>
	...
	##eviewer {
		...
		top: 65px;
		right: -1000px;
		transition: 0.5s opacity, 0.2s right;
		-webkit-transition: 0.5s opacity, 0.2s right;
		-moz-transition: 0.5s opacity, 0.2s right;
		-o-transition: 0.5s opacity, 0.2s right;
	}
		...
		<div id="mailbar">
			<div id="mButtons">
				...
				<cfif qMail.recordCount>
					...
					<input type="Image" name="deleteAll"  src="images/delete_all.png" onClick="refreshBottom();" value="delete" alt="delete all emails" title="delete all emails">
	<!--- 				<img src="images/sep.jpg" width="9" height="40">
...
<table cellspacing=0 cellpadding=3 border=0 id="gradient-style">
<thead>
	<tr class="header">
...
<cfif qMail.recordCount>
	...
	<cfset info = getMail(filename=name, isMailBodyDesired=false)><!--- body can be huge, and isn't needed for list of emails --->
		...
		<tr #vClass#>
			...
			<td class="subject" onClick='getMyEmail("#urlEncodedFormat(name)#")'><cfif structKeyExists(info,"subject") and len(info.subject)>#info.subject#<cfelse>n/a</cfif></td>
			<td class="sender"><cfif structKeyExists(info, "sender") and len(info.sender)>#info.sender#<cfelse>n/a</cfif></td>
			<td class="to"><cfif structKeyExists(info,"to") and len(info.to)>#info.to#<cfelse>n/a</cfif></td>
			<td class="size">#fncFileSize(size)#</td>
			<td class="date"><cfif structKeyExists(info, "sent") and isDate(info.sent)>#dateFormat(info.sent)# #timeFormat(info.sent)#<cfelse>n/a</cfif></td>
<!--- 			<td class="subject" onclick="parent.bottom.location='bottom.cfm?mail=#urlEncodedFormat(name)#';"><a href="bottom.cfm?mail=#urlEncodedFormat(name)#" target="bottom"><cfif structKeyExists(info,"subject") and len(info.subject)>#info.subject#<cfelse>n/a</cfif></a></td>
			<td class="sender" onclick="parent.bottom.location='bottom.cfm?mail=#urlEncodedFormat(name)#';"><cfif structKeyExists(info, "sender") and len(info.sender)>#info.sender#<cfelse>n/a</cfif></td>
			<td class="to" onclick="parent.bottom.location='bottom.cfm?mail=#urlEncodedFormat(name)#';"><cfif structKeyExists(info,"to") and len(info.to)>#info.to#<cfelse>n/a</cfif></td>
			<td class="size" onclick="parent.bottom.location='bottom.cfm?mail=#urlEncodedFormat(name)#';">#fncFileSize(size)#</td>
			<td class="date" onclick="parent.bottom.location='bottom.cfm?mail=#urlEncodedFormat(name)#';"><cfif structKeyExists(info, "sent") and isDate(info.sent)>#dateFormat(info.sent)# #timeFormat(info.sent)#<cfelse>n/a</cfif></td> --->
<!--- 			<td class="subject" filename="#urlEncodedFormat(name)#"><!--- <a href="bottom.cfm?mail=#urlEncodedFormat(name)#" target="bottom"> ---><cfif structKeyExists(info,"subject") and len(info.subject)>#info.subject#<cfelse>n/a</cfif><!--- </a> ---></td>
			<td class="sender"><cfif structKeyExists(info, "sender") and len(info.sender)>#info.sender#<cfelse>n/a</cfif></td>
			<td class="to"><cfif structKeyExists(info,"to") and len(info.to)>#info.to#<cfelse>n/a</cfif></td>
			<td class="size">#fncFileSize(size)#</td>
			<td class="date"><cfif structKeyExists(info, "sent") and isDate(info.sent)>#dateFormat(info.sent)# #timeFormat(info.sent)#<cfelse>n/a</cfif></td> --->
